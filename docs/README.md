...
详细文档：[产品说明](docs/产品说明.md) · [功能介绍](docs/功能介绍.md) · [安装包说明](docs/安装包说明.md)
...
- **股息率 > 3%**：口径为**同花顺「股息率TTM」**。全市场快照读东财 clist **f133**（与同花顺 TTM 同列；锚点：思维列控 2.43、中油资本 1.50）；对已过市盈率关的股票再用同花顺 F10 分红表（`basic.10jqka.com.cn/{code}/bonus.html`）覆盖：只计已实施派现、剔除股利支付率 >200% 的特别分红。**不用 f183**（资金流占比）、**不用**同花顺 526792（振幅）、**不用**腾讯字段 64（年度/含特别分红口径）
...
| 个股/行业研报 | [东方财富研报中心](https://data.eastmoney.com/report/) | 公开接口 |
...
合规措施：仅访问公开披露内容；全局限速（默认任意两次请求间隔 1.5 秒）；不登录、不付费、不绕权限。请仅用于个人研究，遵守各数据源使用条款。
...
- **研报 PDF**：`pdf.dfcfw.com` CDN 按 TLS 指纹区分客户端，程序用 `curl_cffi`（浏览器指纹 HTTP 客户端）下载同一公开 URL，不涉及任何挑战破解。
